...
const Navbar = () => {
  ...
  // 🔥 Show login popup on scroll if not authenticated on home page
  useEffect(() => {
    if (!user && isHomePage) {
      const handleScroll = () => {
  ...
  const handleLoginSuccess = (userData) => {
    // Make sure userData contains the token
    if (typeof userData.token === "string") {
      ...
      login(userData.token); // Pass the token directly to the login function
    } else {
  ...
  return (
    <>
      <nav className="bg-[#f7f1e4] p-4 flex justify-between items-center left-0 w-full z-50 relative">
      ...
      {isAuthModalOpen && (
        <div className="fixed inset-0 bg-[#2c2c2c] bg-opacity-75 flex justify-center items-center z-50">
          <div className="relative p-6 rounded-lg shadow-lg max-w-md w-full">
            <button
              className="absolute top-2 right-2 bg-[#fc372d] text-[#ffff] py-1 px-2 text-sm rounded-xl font-bold"
            ...
            <UserAuthentication
              ...
              onLoginSuccess={handleLoginSuccess} // Ensure this line is added
            />
          </div>
        </div>
      )}
